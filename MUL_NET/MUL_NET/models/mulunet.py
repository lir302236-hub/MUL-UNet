...
class GFFM_AND_FUSION(nn.Module):
    def __init__(self, dim):
        super(GFFM_AND_FUSION, self).__init__()
        self.grad = grad_skip_connection(dim=dim)
    ...
    def forward(self, up_feats, in_feats_edge, in_feats_rest):
        in_feats_edge = self.grad(in_feats_edge)
        in_feats_edge = torch.exp(-torch.mul(in_feats_edge, in_feats_edge))
        input = self.process(in_feats_edge + in_feats_rest)
        input_a, input_b = input.chunk(2, dim=1)
        out = input_a*in_feats_edge + input_b*in_feats_rest
        out = self.fusion(torch.cat([up_feats, out], dim=1))
        return out
...
class MULUNET(nn.Module):
    def __init__(self, kernel_size=5, base_dim=32, depths=[4, 4, 4, 4, 4, 4, 4], conv_layer=ConvLayer, norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=GFFM_AND_FUSION):
        ...
        # edge_and_classify_net
        self.layers_edge_and_classify_net = nn.ModuleList()
        # decoder_net
        self.layers_decoder = nn.ModuleList()
        self.downs_restore = nn.ModuleList()
        self.downs_edge_and_classify = nn.ModuleList()
        self.skips_restore = nn.ModuleList()
        self.skips_edge_and_classify = nn.ModuleList()
        ...
        for i in range(self.half_num + 1):
            self.layers_edge_and_classify_net.append(
                BasicLayer(dim=embed_dims[i], depth=depths[i], net_depth=net_depth, kernel_size=kernel_size,
                           conv_layer=conv_layer, norm_layer=norm_layer, gate_act=gate_act))
        ...
        for i in range(self.half_num):
            ...
            self.downs_edge_and_classify.append(PatchEmbed(patch_size=2, in_chans=embed_dims[i], embed_dim=embed_dims[i + 1]))
            
            self.skips_restore.append(nn.Conv2d(embed_dims[i], embed_dims[i], 1))
            self.skips_edge_and_classify.append(nn.Conv2d(embed_dims[i], embed_dims[i], 1))
        ...
        self.out_edge_step_1 = nn.Sequential(
            nn.Conv2d(192, 3 * 8 ** 2, kernel_size=3, padding=3 // 2, padding_mode='reflect'),
            nn.ReLU(),
            nn.Conv2d(3 * 8 ** 2, 3 * 8 ** 2, kernel_size=3, padding=3 // 2, padding_mode='reflect'),
            nn.ReLU(),
            nn.Conv2d(3 * 8 ** 2, 3 * 8 ** 2, kernel_size=3, padding=3 // 2, padding_mode='reflect'),

        )
        self.out_edge_step_2 = nn.Sequential(
            nn.ReLU(),
            nn.PixelShuffle(8)
        )
        self.fusions_net = nn.Sequential(
            nn.Conv2d(416, 96, kernel_size=3, padding=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        feat_edge_and_classify_in = feat.clone()
        
        skips_restore = []
        skips_classify_and_edge = []
        ...
        for i in range(self.half_num):
            feat_edge_and_classify_in = self.layers_edge_and_classify_net[i](feat_edge_and_classify_in)
            skips_classify_and_edge.append(self.skips_edge_and_classify[i](feat_edge_and_classify_in))
            feat_edge_and_classify_in = self.downs_edge_and_classify[i](feat_edge_and_classify_in)
        
        feat_restore_in = self.layers_restore_net[self.half_num](feat_restore_in)
        feat_edge_and_classify_in = self.layers_edge_and_classify_net[self.half_num](feat_edge_and_classify_in)
        
        out_edge_step1 = self.out_edge_step_1(feat_edge_and_classify_in)
        out_class_step1 = self.classify_1(feat_edge_and_classify_in)
        feat = self.fusions_net(torch.cat([feat_restore_in, out_edge_step1, out_class_step1], dim=1))
        
        # predict edge and clssify
        edge = self.out_edge_step_2(out_edge_step1)
        ...
        for i in range(self.half_num - 1, -1, -1):
            ...
            feat = self.fusions[i](feat, skips_classify_and_edge[i], skips_restore[i])
...
def mulunet():
    return MULUNET(kernel_size=5, base_dim=24, depths=[2, 4, 4, 6, 4, 4, 2], conv_layer=ConvLayer, norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=GFFM_AND_FUSION)
